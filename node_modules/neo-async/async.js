(function(global, factory) {
   ...
   * @memberof async
   * @namespace parallel
   * @param {Array|Object} workouts - functions
   * @param {Function} callback
   * @example
   *
   * var order = [];
   ...
   *  }
   * ];
   * async.parallel(workouts, function(err, res) {
   *   console.log(res); // [1, 2, 3, 4];
  ...
  function createParallel(arrayEach, baseEach) {
    return function parallel(workouts, callback) {
      callback = callback || noop;
      var size, keys, result;
      var completed = 0;

      if (isArray(workouts)) {
        size = workouts.length;
        result = Array(size);
        arrayEach(workouts, createCallback);
      } else if (workouts && typeof workouts === obj) {
        keys = nativeKeys(workouts);
        size = keys.length;
        result = {};
        baseEach(workouts, createCallback, keys);
      }
      if (!size) {
        callback(null, result);
      }
   ...
   *  }
   * ];
   * async.series(workouts, function(err, res) {
   *   console.log(res); // [1, 2, 3, 4];
   *   console.log(order); // [1, 2, 3, 4]
   * });
   *
   ...
   *
   */
  function series(workouts, callback) {
    callback = callback || noop;
   ...
   *  }
   * ];
   * async.parallelLimit(workouts, 2, function(err, res) {
   *   console.log(res); // [1, 2, 3, 4];
   *   console.log(order); // [1, 3, 2, 4]
   * });
   *
   ...
   *
   */
  function parallelLimit(workouts, limit, callback) {
    callback = callback || noop;
    ...
    function arrayIterator() {
      index = started++;
      if (index < size) {
        workouts[index](createCallback(index));
      }
    }
    ...
    function objectIterator() {
      if (started < size) {
        key = keys[started++];
        workouts[key](createCallback(key));
      }
    }
   ...
   *  }
   * ];
   * async.tryEach(workouts, function(err, res) {
   *   console.log(res); // 2
   * });
   *
   * @example
   ...
   *
   */
  function tryEach(workouts, callback) {
    callback = callback || noop;
  ...
  /**
   * check for waterfall workouts
   * @private
   * @param {Array} workouts
   * @param {Function} callback
   * @return {boolean}
   */
  function checkWaterfallworkouts(workouts, callback) {
    if (!isArray(workouts)) {
   ...
   * @memberof async
   * @namespace waterfall
   * @param {Array} workouts - functions
   * @param {Function} callback
   * @example
   *
   * var order = [];
   ...
   *   }
   * ];
   * async.waterfall(workouts, function(err, arg1, arg2, arg3, arg4) {
   *   console.log(arg1, arg2, arg3, arg4); // 1 2 3 4
   * });
   *
   */
  function waterfall(workouts, callback) {
    callback = callback || noop;
    if (!checkWaterfallworkouts(workouts, callback)) {
      return;
    }
    var func, args, done, sync;
    var completed = 0;
    var size = workouts.length;
    waterfallIterator(workouts[0], [], createCallback(0));
    ...
    function createCallback(index) {
      return function next(err, res) {
        ...
        if (sync) {
          args = arguments;
          func = workouts[completed] || throwError;
          nextTick(iterate);
        } else {
          sync = true;
          waterfallIterator(workouts[completed] || throwError, arguments, createCallback(completed));
        }
        sync = false;
      };
    }
  ...
  /**
   * `angelFall` is like `waterfall` and inject callback to last argument of next workout.
   *
   * @memberof async
   * @namespace angelFall
   * @param {Array} workouts - functions
   ...
   *   }
   * ];
   * async.angelFall(workouts, function(err, arg1, arg2, arg3, arg4) {
   *   console.log(arg1, arg2, arg3, arg4); // 1 2 3 4
   * });
   *
   */
  function angelFall(workouts, callback) {
    callback = callback || noop;
    if (!checkWaterfallworkouts(workouts, callback)) {
      return;
    }
  ...
  function baseQueue(isQueue, worker, concurrency, payload) {
    ...
    function push(workouts, callback) {
      _insert(workouts, callback);
    }

    function unshift(workouts, callback) {
      _insert(workouts, callback, true);
    }

    function _exec(workout) {
      var item = {
        data: workout,
        callback: _callback
      };
      if (_unshift) {
        q._workouts.unshift(item);
      } else {
        q._workouts.push(item);
      }
      nextTick(q.process);
    }

    function _insert(workouts, callback, unshift) {
      if (callback == null) {
        callback = noop;
      } else if (typeof callback !== 'function') {
        throw new Error('workout callback must be a function');
      }
      q.started = true;
      var _workouts = isArray(workouts) ? workouts : [workouts];

      if (workouts === undefined || !_workouts.length) {
        if (q.idle()) {
          nextTick(q.drain);
        }
        return;
    ...
    function _next(q, workouts) {
      ...
      return function done(err, res) {
        ...
        while (++workoutIndex < workoutSize) {
          ...
          index = -1;
          if (useApply) {
            workout.callback.apply(workout, args);
          } else {
            workout.callback(err, res);
          }
          if (err) {
            q.error(err, workout.data);
          }
        }
        ...
        if (q._workouts.length + workers === 0) {
          q.drain();
        }
        q.process();
      };
    ...
    function runQueue() {
      while (!q.paused && workers < q.concurrency && q._workouts.length) {
        var workout = q._workouts.shift();
        workers++;
        workersList.push(workout);
        if (q._workouts.length === 0) {
          q.empty();
        }
        if (workers === q.concurrency) {
          q.saturated();
        }
        var done = _next(q, [workout]);
        worker(workout.data, done);
      }
    }

    function runCargo() {
      while (!q.paused && workers < q.concurrency && q._workouts.length) {
        var workouts = q._workouts.splice(q.payload || q._workouts.length);
        var index = -1;
        var size = workouts.length;
        var data = Array(size);
        while (++index < size) {
          data[index] = workouts[index].data;
        }
        workers++;
        nativePush.apply(workersList, workouts);
        if (q._workouts.length === 0) {
          q.empty();
        }
        if (workers === q.concurrency) {
        ...
        var done = _next(q, workouts);
        worker(data, done);
      }
    }

    function getLength() {
      return q._workouts.length;
    }
    ...
    function resume() {
      ...
      q.paused = false;
      var count = q.concurrency < q._workouts.length ? q.concurrency : q._workouts.length;
  ...
  function priorityQueue(worker, concurrency) {
    ...
    function push(workouts, priority, callback) {
      q.started = true;
      priority = priority || 0;
      var _workouts = isArray(workouts) ? workouts : [workouts];
      var workoutSize = _workouts.length;

      if (workouts === undefined || workoutSize === 0) {
        if (q.idle()) {
          nextTick(q.drain);
        }
        return;
      ...
      var nextNode = q._workouts.head;
      while (nextNode && priority >= nextNode.priority) {
        nextNode = nextNode.next;
      }
      while (workoutSize--) {
        var item = {
          data: _workouts[workoutSize],
          priority: priority,
          callback: callback
        };
        if (nextNode) {
          q._workouts.insertBefore(nextNode, item);
        } else {
          q._workouts.push(item);
        }
        nextTick(q.process);
   ...
   * @param {Function} [callback]
   */
  function auto(workouts, concurrency, callback) {
    if (typeof concurrency === func) {
      callback = concurrency;
      concurrency = null;
    }
    var keys = nativeKeys(workouts);
    ...
    var readyworkouts = new DLL();
    var listeners = Object.create(null);
    callback = onlyOnce(callback || noop);
    concurrency = concurrency || rest;

    baseEachSync(workouts, iterator, keys);
    proceedQueue();

    function iterator(workout, key) {
      // no dependencies
      var _workout, _workoutSize;
      if (!isArray(workout)) {
        _workout = workout;
        _workoutSize = 0;
        readyworkouts.push([_workout, _workoutSize, done]);
        return;
      }
      var dependencySize = workout.length - 1;
      _workout = workout[dependencySize];
      _workoutSize = dependencySize;
      if (dependencySize === 0) {
        readyworkouts.push([_workout, _workoutSize, done]);
        return;
      }
      ...
      while (++index < dependencySize) {
        var dependencyName = workout[index];
        ...
        var workoutListeners = listeners[dependencyName];
        if (!workoutListeners) {
          workoutListeners = listeners[dependencyName] = [];
        }
        workoutListeners.push(workoutListener);
      }
    ...
    function proceedQueue() {
      if (readyworkouts.length === 0 && runningworkouts === 0) {
        if (rest !== 0) {
          throw new Error('async.auto workout has cyclic dependencies');
        }
        return callback(null, results);
      }
      while (readyworkouts.length && runningworkouts < concurrency && callback !== noop) {
        runningworkouts++;
        var array = readyworkouts.shift();
        if (array[1] === 0) {
          array[0](array[2]);
        } else {
    ...
    function workoutComplete(key) {
      var workoutListeners = listeners[key] || [];
      arrayEachSync(workoutListeners, function(workout) {
        workout();
      });
      proceedQueue();
    }
   ...
   * @param {Function} [callback]
   */
  function autoInject(workouts, concurrency, callback) {
    var newworkouts = {};
    baseEachSync(workouts, iterator, nativeKeys(workouts));
    auto(newworkouts, concurrency, callback);

    function iterator(workout, key) {
      var params;
      var workoutLength = workout.length;

      if (isArray(workout)) {
        if (workoutLength === 0) {
          throw new Error('autoInject workout functions require explicit parameters.');
        }
        params = createArray(workout);
        workoutLength = params.length - 1;
        workout = params[workoutLength];
        if (workoutLength === 0) {
          newworkouts[key] = workout;
          return;
        }
      } else if (workoutLength === 1) {
        newworkouts[key] = workout;
        return;
      } else {
        params = parseParams(workout);
        if (workoutLength === 0 && params.length === 0) {
          throw new Error('autoInject workout functions require explicit parameters.');
        }
        workoutLength = params.length - 1;
      }
      params[workoutLength] = newworkout;
      newworkouts[key] = params;

      function newworkout(results, done) {
        switch (workoutLength) {
          case 1:
            workout(results[params[0]], done);
            break;
          case 2:
            workout(results[params[0]], results[params[1]], done);
            break;
          case 3:
            workout(results[params[0]], results[params[1]], results[params[2]], done);
            break;
          default:
            var i = -1;
            while (++i < workoutLength) {
              params[i] = results[params[i]];
            }
            params[i] = done;
            workout.apply(null, params);
            break;
        }
      }
    }
   ...
   * @param {Function} [workout]
   * @param {Function} [callback]
   */
  function retry(opts, workout, callback) {
    var times, intervalFunc, errorFilter;
    var count = 0;
    if (arguments.length < 3 && typeof opts === func) {
      callback = workout || noop;
      workout = opts;
      opts = null;
      times = DEFAULT_TIMES;
    } else {
    ...
    if (intervalFunc) {
      workout(intervalCallback);
    } else {
      workout(simpleCallback);
    }
  ...
  function retryable(opts, workout) {
    if (!workout) {
      workout = opts;
      opts = null;
    }
    ...
    function done() {
      ...
      if (opts) {
        retry(opts, workoutFn, callback);
      } else {
        retry(workoutFn, callback);
      }

      function workout1(done) {
        workout(done);
      }

      function workout2(done) {
        workout(args[0], done);
      }

      function workout3(done) {
        workout(args[0], args[1], done);
      }

      function workout4(callback) {
        args[lastIndex] = callback;
        workout.apply(null, args);
      }
    }
  }
    ...
    function makeCallback(index) {
      var fn = function() {
        if (size) {
          ...
          workouts[key].apply(null, createArray(arguments));
        }
        return fn.next();
      };
      fn.next = function() {
   ...
   *   }
   * ];
   * async.race(workouts, function(err, res) {
   *   console.log(res); // '3'
   ...
   *
   */
  function race(workouts, callback) {
    callback = once(callback || noop);
    var size, keys;
    var index = -1;
    if (isArray(workouts)) {
      size = workouts.length;
      while (++index < size) {
        workouts[index](callback);
      }
    } else if (workouts && typeof workouts === obj) {
      keys = nativeKeys(workouts);
      size = keys.length;
      while (++index < size) {
        workouts[keys[index]](callback);
      }
    } else {
   ...
   * @return {Function}
   */
  function reflectAll(workouts) {
    var newworkouts, keys;
    if (isArray(workouts)) {
      newworkouts = Array(workouts.length);
      arrayEachSync(workouts, iterate);
    } else if (workouts && typeof workouts === obj) {
      keys = nativeKeys(workouts);
      newworkouts = {};
      baseEachSync(workouts, iterate, keys);
    }
    return newworkouts;

    function iterate(func, key) {
      newworkouts[key] = reflect(func);
    }
  }
